	...
	<div class="orderd">
		...
		<div class="body">
			...
			<div class="com_box" v-if="Object.keys(service_info).length">
                ...
                <div v-if="service_info.row && service_info.row.service_value_unit_id == 2">
                  价格已包含自营清洁剂
                </div>
							</div>
...
<script>
  ...
  import periodicReservation from '@/components/periodicReservation'
      ...
      periodicReservation,
		...
		created () { //生命周期
          ...
          this.service_item.push({
			...
			this.init();
			this.getAddressList();
			console.log('1',this.service_info)
    },
		watch: {
      long(newVal) {
    ...
    return this.getOrder(this.selectOrder)
		...
		methods: { //方法
      format (index) {
        // this.money += this.long *
        // lists

        consolr.log(this.lists)
        let val = this.lists

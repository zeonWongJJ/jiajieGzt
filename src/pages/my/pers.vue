		...
		<div class="top_img">
			...
			<div class="ri_inpu uploa">
				...
				<div class="imgs" v-if="path_img == ''">
					...
					<img :src="uploadFileUrl + infoList.user_pic" v-else />
				</div>
		...
		methods: { //方法
			loadImg(filer) { //图片上传方法
				...
				that.$fetch('uploadBase', lists).then(rs => {
					that.infoList.user_pic = rs.path;
					that.$toast('上传成功');
				})
			},
			userInfo() {
			...
			addbuts() {
				...
				Object.keys(that.user_info).forEach(key =>{
					if(that.user_info[key] != that.infoList[key]){
						lists[key] = that.infoList[key]
					}
				})

	...
	<div class="credit">
		...
		<div class="body">
			...
			<div class="center">
				...
				<div class="ul">
					<scroller :on-infinite="infinite" ref="scroller_cred">
	...
	export default{
		...
		methods:{
            ...
            if(rs.length != 10){//如果数据长度小于10证明下次请求没有数据
              that.$refs.scroller_cred.finishInfinite(true);//执行组件完成上拉方法(true代表没有数据)
              that.end = true
            }else{
              that.$refs.scroller_cred.finishInfinite(false);//执行组件完成上拉方法(true代表没有数据)
            }

<template>
	<div class="balance">
        ...
        <div class="user_coupon" style="text-align: center;padding-bottom: .1rem;color: #f00;">包含返现的{{Number(user_coupon).toFixed(2)}}元，返现金额不能提现</div>
				<div class="row">
			...
			<div class="center">
				...
				<div class="ul">
					<scroller :on-infinite="infinite" ref="scroller_balance">
					  <!-- content goes here -->
	...
	export default{
		...
		methods:{
	    	...
	    	init(){//初始化请求
				...
				that.$fetch('user_info_get', {is_withdraw: 1}, '', 'GET').then(rs =>{
          that.value = Number(rs.user_balance).toFixed(2)
          that.user_coupon = rs.user_coupon
				})
				that.$fetch('user_balance_list', lists).then(rs =>{
          that.page++//请求页数自加
          that.balance = rs;//覆盖本地数据
          if(rs.length != 10){//如果数据长度小于10证明下次请求没有数据
            that.$refs.scroller_balance.finishInfinite(true);//执行组件完成上拉方法(true代表没有数据)
            that.end = true
          }else{
            that.$refs.scroller_balance.finishInfinite(false);//执行组件完成上拉方法(true代表没有数据)
          }
